...
The polygon's list of polygons. Every polygon is represented by a :ref:`PoolArray<class_PoolIntArray>` where each :ref:`int<class_int>` is an index to a point in ``polygon``. If empty, this property will be ignored and the polygon will be composed of all points in ``polygon``.
